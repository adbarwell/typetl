...
binary_test_() ->
    {foreach,
     fun() ->
             ...
             Xs_io_bin = binarylists:converter(Xs_io),
             Xs_term_bin = binarylists:converter(Xs_term),
     ...
     [
      fun({Xs_io, Xs_term, Xs_io_bin, Xs_term_bin, Ys_io, Ys_term}) ->
              [
               ?_assertEqual(Xs_io, (binarylists:to_list(Xs_io_bin))),
               ?_assertEqual(Xs_term, (binarylists:to_list(Xs_term_bin))),
               begin
                   F1 = fun(X) -> X < 50 end,
                   F2 = fun(X) -> X < 200 end,
               ...
               ?_assertEqual((lists:append(Xs_io, Ys_io)),
                             (binarylists:to_list(binarylists:append(Xs_io_bin, Ys_io)))),
               ?_assertEqual((lists:append(Xs_io, Ys_term)),
                             (binarylists:to_list(binarylists:append(Xs_io_bin, Ys_term)))),
               ?_assertEqual((lists:append(Xs_term, Ys_io)),
                             (binarylists:to_list(binarylists:append(Xs_term_bin, Ys_io)))),
               ?_assertEqual((lists:append(Xs_term, Ys_term)),
                             (binarylists:to_list(binarylists:append(Xs_term_bin, Ys_term)))),
               ?_assertEqual((lists:delete(50, Xs_io)),
                             (binarylists:to_list(binarylists:delete(50, Xs_io_bin)))),
               ?_assertEqual((lists:delete({'test', 50}, Xs_term)),
                             (binarylists:to_list(binarylists:delete({'test', 50}, Xs_term_bin)))),
               begin
                   F5 = fun(X) -> X < 50 end,
                   F6 = fun(X) -> element(1, X) =:= 'test' end,
                   F7 = fun(X) -> element(2, X) < 50 end,
                   [?_assertEqual((lists:dropwhile(F5, Xs_io)),
                                  (binarylists:to_list(binarylists:dropwhile(F5, Xs_io_bin)))),
                    ?_assertEqual((lists:dropwhile(F6, Xs_term)),
                                  (binarylists:to_list(binarylists:dropwhile(F6, Xs_term_bin)))),
                    ?_assertEqual((lists:dropwhile(F7, Xs_term)),
                                  (binarylists:to_list(binarylists:dropwhile(F7, Xs_term_bin)))),
                    ?_assertEqual((lists:filter(F5, Xs_io)),
                                  (binarylists:to_list(binarylists:filter(F5, Xs_io_bin)))),
                    ?_assertEqual((lists:filter(F6, Xs_term)),
                                  (binarylists:to_list(binarylists:filter(F6, Xs_term_bin)))),
                    ?_assertEqual((lists:filter(F7, Xs_term)),
                                  (binarylists:to_list(binarylists:filter(F7, Xs_term_bin))))
                   ]
               end,
               ?_assertEqual((lists:flatten(Xs_io)),
                             (binarylists:to_list(binarylists:flatten(Xs_io_bin)))),
               begin
                   Xs_deep = lists:duplicate(10, Xs_term),
                   Xs_deep_bin = binarylists:converter(Xs_deep),
                   ?_assertEqual((lists:flatten(Xs_deep)),
                                 (binarylists:to_list(binarylists:flatten(Xs_deep_bin))))
               end,
               begin
               ...
               begin
                   ...
                   [
                    ?_assertEqual((lists:map(F12, Xs_io)),
                                  (binarylists:to_list(binarylists:map(F12, Xs_io_bin)))),
                    ?_assertEqual((lists:map(F13, Xs_io)),
                                  (binarylists:to_list(binarylists:map(F13, Xs_io_bin)))),
                    ?_assertEqual((lists:map(F14, Xs_term)),
                                  (binarylists:to_list(binarylists:map(F14, Xs_term_bin)))),
                    ?_assertEqual((lists:map(F15, Xs_term)),
                                  (binarylists:to_list(binarylists:map(F15, Xs_term_bin))))
                   ]
               end,
               ...
               ?_assertEqual((lists:merge(Xs_io, Ys_io)),
                             (binarylists:to_list(binarylists:merge(Xs_io_bin, Ys_io)))),
               ?_assertEqual((lists:merge(Xs_io, Ys_term)),
                             (binarylists:to_list(binarylists:merge(Xs_io_bin, Ys_term)))),
               ?_assertEqual((lists:merge(Xs_term, Ys_io)),
                             (binarylists:to_list(binarylists:merge(Xs_term_bin, Ys_io)))),
               ?_assertEqual((lists:merge(Xs_term, Ys_term)),
                             (binarylists:to_list(binarylists:merge(Xs_term_bin, Ys_term)))),
               ...
               ?_assertEqual((lists:nthtail(50, Xs_io)),
                             (binarylists:to_list(binarylists:nthtail(50, Xs_io_bin)))),
               ?_assertEqual((lists:nthtail(50, Xs_term)),
                             (binarylists:to_list(binarylists:nthtail(50, Xs_term_bin)))),
               ...
               begin
                   ...
                   [
                    ?_assertEqual((tuple_to_list(lists:partition(F16, Xs_io))),
                                  (lists:map(fun binarylists:to_list/1,
               ...
               ?_assertEqual((lists:reverse(Xs_io)),
                             (binarylists:to_list(binarylists:reverse(Xs_io_bin)))),
               ?_assertEqual((lists:reverse(Xs_term)),
                             (binarylists:to_list(binarylists:reverse(Xs_term_bin)))),
               ?_assertEqual((lists:sort(Xs_io)),
                             (binarylists:to_list(binarylists:sort(Xs_io_bin)))),
               ?_assertEqual((lists:sort(lists:reverse(Xs_io))),
                             (binarylists:to_list(binarylists:sort(binarylists:converter(lists:reverse(Xs_io)))))),
               ?_assertEqual((lists:sort(Xs_term)),
                             (binarylists:to_list(binarylists:sort(Xs_term_bin)))),
               ?_assertEqual((lists:sort(lists:reverse(Xs_term))),
                             (binarylists:to_list(binarylists:sort(binarylists:converter(lists:reverse(Xs_term)))))),
               ?_assertEqual((tuple_to_list(lists:split(50, Xs_io))),
                             (lists:map(fun binarylists:to_list/1,
                                        tuple_to_list(binarylists:split(50, Xs_io_bin))))),
               ?_assertError(badarg, (lists:map(fun binarylists:to_list/1,
               ...
               ?_assertEqual((lists:sublist(Xs_io, 5, 10)),
                             (binarylists:to_list(binarylists:sublist(Xs_io_bin, 5, 10)))),
               ?_assertEqual((lists:sublist(Xs_term, 5, 10)),
                             (binarylists:to_list(binarylists:sublist(Xs_term_bin, 5, 10)))),
               ?_assertEqual((lists:sublist(Xs_io, 95, 50)),
                             (binarylists:to_list(binarylists:sublist(Xs_io_bin, 95, 50)))),
               ?_assertEqual((lists:sublist(Xs_term, 95, 50)),
                             (binarylists:to_list(binarylists:sublist(Xs_term_bin, 95, 50)))),
               begin
                   F20 = fun(X) -> X < 50 end,
                   F21 = fun(X) -> X < 200 end,
                   ...
                   [
                    ?_assertEqual((lists:takewhile(F20, Xs_io)),
                                  (binarylists:to_list(binarylists:takewhile(F20, Xs_io_bin)))),
                    ?_assertEqual((lists:takewhile(F21, Xs_io)),
                                  (binarylists:to_list(binarylists:takewhile(F21, Xs_io_bin)))),
                    ?_assertEqual((lists:takewhile(F22, Xs_term)),
                                  (binarylists:to_list(binarylists:takewhile(F22, Xs_term_bin)))),
                    ?_assertEqual((lists:takewhile(F23, Xs_term)),
                                  (binarylists:to_list(binarylists:takewhile(F23, Xs_term_bin))))
                   ]
               end,
               ?_assertEqual((lists:zip(Xs_io, Xs_term)),
                             (binarylists:to_list(binarylists:zipl(Xs_io_bin, Xs_term)))),
               ?_assertEqual((lists:zip(Xs_io, Ys_io)),
                             (binarylists:to_list(binarylists:zipl(Xs_io_bin, Ys_io)))),
               ?_assertEqual((lists:zip(Xs_term, Xs_io)),
                             (binarylists:to_list(binarylists:zipl(Xs_term_bin, Xs_io)))),
               ?_assertEqual((lists:zip(Xs_term, Ys_term)),
                             (binarylists:to_list(binarylists:zipl(Xs_term_bin, Ys_term)))),
               ?_assertEqual((lists:zip(Xs_term, Xs_io)),
                             (binarylists:to_list(binarylists:zipr(Xs_term, Xs_io_bin)))),
               ?_assertEqual((lists:zip(Ys_io, Xs_io)),
                             (binarylists:to_list(binarylists:zipr(Ys_io, Xs_io_bin)))),
               ?_assertEqual((lists:zip(Xs_io, Xs_term)),
                             (binarylists:to_list(binarylists:zipr(Xs_io, Xs_term_bin)))),
               ?_assertEqual((lists:zip(Ys_term, Xs_term)),
                             (binarylists:to_list(binarylists:zipr(Ys_term, Xs_term_bin)))),
               ?_assertEqual((lists:zip(Xs_io, Ys_io)),
                             (binarylists:to_list(binarylists:zip(Xs_io_bin,
                                                                  binarylists:converter(Ys_io))))),
               ?_assertEqual((lists:zip(Xs_term, Ys_term)),
                             (binarylists:to_list(binarylists:zip(Xs_term_bin,
                                                                  binarylists:converter(Ys_term))))),
               begin
                   F24 = fun(X, Y) -> X + Y end,
